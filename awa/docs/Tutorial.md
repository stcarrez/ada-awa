...
that is a free modelization tool that works pretty well.  For the  [ArgoUML](http://argouml.tigris.org/)
setup, we will use two profiles:
...
directory after [Dynamo](https://github.com/stcarrez/dynamo) and AWA are installed.  To configure [ArgoUML](http://argouml.tigris.org/),
...
The `Makefile.in` that was generated by the [Dynamo](https://github.com/stcarrez/dynamo) `create-project`
command must be updated to setup a number of generation arguments for
the UML to Ada code generator.  Edit the `Makefile.in` to change `DYNAMO_ARGS` into:

```
DYNAMO_ARGS=--package Atlas.Reviews.Models db uml/atlas.zargo
```
The `--package` option tells [Dynamo](https://github.com/stcarrez/dynamo) to generate only the model for the
